...
export const DocumentPreview: React.FC<DocumentPreviewProps> = ({
  ...
  const previewSourceKey = `${selectedDocIndex}:${fileBase64 ?? ""}`;
  const [syncedPreviewSourceKey, setSyncedPreviewSourceKey] =
    useState(previewSourceKey);

  if (previewSourceKey !== syncedPreviewSourceKey) {
    setSyncedPreviewSourceKey(previewSourceKey);
    setDocId(null);
  }
  ...
  const handleViewerTabChange = useCallback(
    (index: number) => {
      ...
      if (index === 1 && fileBase64 && !docId) {
        void uploadForExternalViewer();
      }
    },
    [docId, fileBase64, uploadForExternalViewer],
  );
    ...
    >
      ...
      {hasDocuments && (
        <TabGroup
          ...
          onChange={handleViewerTabChange}
        >
