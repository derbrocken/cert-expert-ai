...
const DEFAULT_GLOBAL: GlobalProperties = {
  companyName: "",
  companyEmail: "",
  companyAddress: "",
};

export function loadGlobalExportSettings(): GlobalProperties {
  return loadEmployeeQueue()?.globalProps ?? DEFAULT_GLOBAL;
}

export function saveGlobalExportSettings(globalProps: GlobalProperties): void {
  const existing = loadEmployeeQueue();
  saveEmployeeQueue({
