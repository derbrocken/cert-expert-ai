...
const STRATEGIE_STATUS = {
  done: { label: "Erreicht", class: "st-done" },
  in_progress: { label: "Läuft", class: "st-progress" },
  not_started: { label: "Offen", class: "st-open" },
};

function renderStrategieBoard() {
  const root = $("#strategie-board");
  if (!root) return;
  const s = state?.strategie;
  if (!s?.corporate_guide) {
    root.innerHTML =
      '<p class="empty">Strategie-Daten fehlen — <code>hq/00_Dashboard/strategie_dashboard.json</code> anlegen und Build ausführen.</p>';
    return;
  }
  const g = s.corporate_guide;
  const v = s.vision_2036 || {};
  const rm = s.roadmap_2026 || {};
  const phases = rm.phases || [];
  const tactical = s.tactical_goals || [];
  const strategic = s.strategic_goals || [];

  const statusDot = (status) => {
    const m = STRATEGIE_STATUS[status] || STRATEGIE_STATUS.not_started;
    return `<span class="st-dot ${m.class}" title="${esc(m.label)}"></span>`;
  };

  const valuesHtml = (g.values || [])
    .map((x) => `<span class="st-value-chip">${esc(x)}</span>`)
    .join("");

  const pillarsHtml = (v.pillars || [])
    .map((x) => `<li>${esc(x)}</li>`)
    .join("");
  ...
  const phasesHtml = phases
    .map(
      (p) => `
      <article class="st-phase-card ${STRATEGIE_STATUS[p.status]?.class || "st-open"}">
        <div class="st-phase-head">${statusDot(p.status)}<span class="st-phase-n">Phase ${p.n}</span></div>
        <h4>${esc(p.title)}</h4>
      </article>`
    )
    .join("");
  ...
  const tacticalHtml = tactical
    .map(
      (t, i) => `
      <li class="st-tactical-item">
        ${statusDot(t.status)}
        <span class="st-tactical-n">${i + 1}</span>
        <span>${esc(t.title)}</span>
      </li>`
    )
    .join("");

  const strategicHtml = strategic
    .map(
      (sg) => `
      <li class="st-strategic-item">
        <strong>${esc(sg.title)}</strong>
        <span class="theme-meta">${esc(sg.horizon || "")}</span>
      </li>`
    )
    .join("");

  const strandsHtml = (rm.parallel_strands || [])
    .map(
      (st) => `
      <div class="st-strand">
        <h4>${esc(st.title)}</h4>
  ...
  root.innerHTML = `
    <header class="st-hero">
      <div class="st-hero-visual">
        <img src="/assets/mantis-mark.png" alt="Cert-Expert" class="st-hero-mark" />
      </div>
      <div class="st-hero-text">
        <p class="st-eyebrow">Unternehmensführung · QM-Ebene</p>
        <h2>${esc(g.title || "Wofür wir stehen")}</h2>
        <p class="st-leitbild">${esc(g.leitbild)}</p>
        <p class="st-mission">${esc(g.mission)}</p>
        <p class="st-philosophy">${esc(g.philosophy)}</p>
        <div class="st-values">${valuesHtml}</div>
      </div>
    </header>

    <nav class="st-qm-ladder" aria-label="QM-Logik">
      <div class="st-qm-step st-qm-vision"><span>Vision 2036</span></div>
      <div class="st-qm-step st-qm-strategic"><span>Strategische Ziele</span></div>
      <div class="st-qm-step st-qm-tactical"><span>Taktisch 2026–2028</span></div>
      <div class="st-qm-step st-qm-operative"><span>Operativ · Übersicht oben</span></div>
    </nav>

    <section class="st-panel st-vision-panel">
      <header class="st-panel-head">
        <h3>${esc(v.title || "Vision 2036")}</h3>
        <p class="st-panel-sub">${esc(v.headline || "")}</p>
      </header>
      <div class="st-horizon">
        <div class="st-horizon-track">
          <span class="st-horizon-point is-now"><b>2026</b><small>Heute</small></span>
          <span class="st-horizon-line"></span>
          <span class="st-horizon-point"><b>2028</b><small>Taktik</small></span>
          <span class="st-horizon-line"></span>
          <span class="st-horizon-point is-future"><b>2036</b><small>Vision</small></span>
        </div>
      </div>
      <p class="st-summary">${esc(v.summary || "")}</p>
      <ul class="st-pillars">${pillarsHtml}</ul>
      <p class="st-mantis-quote">${esc(v.mantis || "")}</p>
    </section>
    ...
    <div class="st-grid-two">
      <section class="st-panel">
        ...
        <ul class="st-strategic-list">${strategicHtml}</ul>
      </section>
      <section class="st-panel">
        <header class="st-panel-head">
          <h3>Taktische Ziele 2026–2028</h3>
        </header>
        <ul class="st-tactical-list">${tacticalHtml}</ul>
      </section>
    </div>

    <section class="st-panel st-roadmap-panel">
      <header class="st-panel-head">
        <h3>Roadmap 2026</h3>
        <p class="st-nordstern">${esc(rm.nordstern || "")}</p>
        <p class="st-motto">${esc(rm.motto || "")}</p>
      </header>
      <div class="st-strands">${strandsHtml}</div>
      <div class="st-phases-grid">${phasesHtml}</div>
    </section>

    <p class="st-operative-hint">${esc(s.operative_note || "")}</p>
  `;
}
...
function setView(view, scrollId = null) {
  ...
  $("#strategie-zone")?.classList.toggle("hidden", isFinanzView);
  ...
  document.querySelectorAll(".side-link[data-view]").forEach((el) => {
    ...
    if (v === "finance") {
      ...
      el.classList.toggle("is-active", view === "day" && !isFinanzView);
    }
  });
...
async function init() {
  ...
  $("#btn-jump-strategie")?.addEventListener("click", () => {
    setView("day");
    document.getElementById("strategie-zone")?.scrollIntoView({ behavior: "smooth", block: "start" });
  });
  ...
  document.querySelectorAll(".side-link[data-view]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      setView(view);
      if (view === "day") {
        document.getElementById("pin-zone")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }
    });
  });
