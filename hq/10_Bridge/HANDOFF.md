...
> **▶ JETZT (Planer-Chat, 2026-06-08 ~16:40):** EK/FK-Refinement `e1899dd` **Planer-abgenommen ✅** (unabhängig re-verifiziert: tsc 0 · Engine 27/27 · EC-09-ZIP 200 · Migration live · norm-konform, keine neue CL/UE). **Nächste Bau-Queue = `CURSOR_NAECHSTE_QUEUE.md`** (A Norm-Cross-Check → B read-only Übersicht → C Termin-Planung → D Modell-Trennung). **A erledigt (Planer):** `NORM_CROSSCHECK_SCHULUNGSKATALOG.md` — Katalog-UE↔CL gemappt; Befunde: 🔴 Veranstaltung-FK Katalog 16 vs. Norm 24 (CL-20) · 🔴 ÖPV ohne CL (Lücke) · Module = Curriculum, nicht Norm. Engine bleibt norm-konform, keine UE-Änderung ohne Mark-Klärung (3 Fragen in der Cross-Check-Doku §4). **B LÄUFT (Subagent):** read-only Akte-/Vorzeige-Übersicht nach `CURSOR_UEBERSICHT_READONLY_AUFTRAG.md` (reine Präsentation, EC-09/EC-10 gewahrt). Nach B-Abnahme → C.
