...
> ## ▶ HIER STARTEN — AKTUELLER STAND (2026-06-21) · ÜBERGABE an Mark/neuen Planer
> **▶ JETZT — GENERATOR-UX Slice A+C LIVE (2026-06-21), `cb8313d`.** Marks Live-Befund (3 Punkte) → Konzept `AKTE_GENERATOR_UEBERGANG_KONZEPT.md` (Mark-Gate-Entscheide gesetzt). **Slice A** = Generator entdoppelt: doppelter Navbar-Eintrag „Dokument-Generator"/`/model-creator` entfernt (Route weg, Komponente nach `app/generator/`), eine „Generator"-Leiste mit Umschalter Firma/Mitarbeiter; tote Verweise (Dashboard 2×, DocumentForm) auf `/generator` umgebogen. **Slice C** = Schulungs-von–bis je Person direkt im Generator-Datumsblock (neue Sektion, Quelle `trainingPlan` via `resolveAssignedSchulungDocs`, EINE Quelle mit der Terminplanung, Dokumentdatum=letzter Tag). **Gates je Slice: tsc 0 · next build grün.** Kein Schema-Change. **DEPLOYT** `27aa523`→`cb8313d` (Backup `dev-20260621-165135.db`, kein db push; Live: /login 200, /→307, /model-creator→404 eingeloggt, /generator+/employee-automation 200). **OFFEN:** (1) Mark Browser-Klick-Abnahme A/C + ZIP (EC-09). (2) **Slice B (Vormerken in „Offene Punkte")** wartet auf Marks **Vorlagen↔Rolle-Mapping** (norm-belegtes requirementId→templateDocId; vorgemerkt→verschwindet aus Liste, EC-10: ≠ erledigt). **⚠️ Git-Lehre 2026-06-20:** nie `git add -A` (DSGVO — versehentlich gepushtes prisma-DB-Backup; `.gitignore` jetzt `prisma/**/*.db*`+`*.db.bak*`; bereinigt via Force-Push). **Push-Remote = SSH** (`git@github.com:derbrocken/cert-expert-ai.git`; altes HTTPS-Token abgelaufen).
> **▶ VORHER — LOGIN / SITE-GATE LIVE DEPLOYT (2026-06-20).** **Deploy durch:** `20e6bf9`→`27aa523` auf cos.cert-expert.de (DB-Backup `dev-20260620-211608.db` + `db:push` additiv M6 „in sync" + `npm ci`/build grün + Restart active). **Live verifiziert:** `/login` 200 · `/`→307 `/login?next=` · Webhook 405 (offen, nicht gesperrt) · falsches PW 401 · richtiges PW (`TeamTurtle26!`) 200+Cookie · geschützt mit Cookie 200. Mit live gegangen ist der ganze Nachzug seit 13.06. (M3–M7, Generator G1/G1b/G4, Schulungs-Fix). **Push-Zugang repariert:** altes HTTPS-Token (iCloud-Keychain) abgelaufen → Remote auf **SSH** umgestellt (`git@github.com:…`, Key bei GitHub hinterlegt), pushen läuft wieder. **OFFEN (Mark):** Browser-Klick-Abnahme (Login-Maske + ZIP-Export eingeloggt = EC-09) + visuelle Sicht der neuen Akte-/Generator-Features. Details `HETZNER_DEPLOY.md` (oberster Block). — *Bau-Historie:* **GEBAUT (2026-06-20):** Marks Auftrag „bau mir Login für cos.cert-expert.de" → Gate-Entscheid (C-10, von Mark): **EIN gemeinsames Passwort** sperrt die GANZE App (schließt das DSGVO-Loch: App lief öffentlich ohne Auth mit Personendaten) · **leichtes eigenes Cookie-Gate**, keine neue Lib. **Gebaut:** `lib/auth.ts` (isomorphe Web-Crypto-Auth: Token signieren/prüfen, Konstantzeit-Vergleich — läuft in Edge **und** Node) · `proxy.ts` (Next-16-`proxy`, ex-`middleware`; schützt alle Routen, OFFEN nur `/login` + `/api/auth/*` + `/api/webhooks/*` → **Tally-Webhook unberührt, eigene Signaturprüfung, EC-09/Intake intakt**) · `app/login/page.tsx` (Brand-Vermillion, Logo, PW-Toggle) · `app/api/auth/login|logout/route.ts` (PW gegen `APP_PASSWORD` konstantzeitig → signiertes httpOnly-Cookie 7 Tage) · Navbar „Abmelden" · `.env.example` (`APP_PASSWORD`+`AUTH_SECRET` dokumentiert). **Gates: tsc 0 · `next build` grün** (Deprecation-Warnung weg nach middleware→proxy-Migration; `proxy`-Runtime=nodejs, Web-Crypto ok). **Live-Smoke (curl gg. Dev) alle grün:** ungeschützt→307 `/login?next=` · `/login`→200 · Webhook erreichbar (eigener „Invalid Tally signature", NICHT Gate) · geschützte API ohne Cookie→401 „Nicht angemeldet" · falsches PW→401 · richtiges PW→200+Cookie · mit Cookie Seite+API→200 · nach Logout→307. Kein Schema-Change, kein `db push`. **▶ OFFEN für Mark:** (1) **Browser-Klick-Abnahme** (Login-Formular + ein ZIP-Export eingeloggt = EC-09-Klick) — Code-Gates+curl grün, visueller Klick steht aus. (2) **DEPLOY braucht `APP_PASSWORD`+`AUTH_SECRET` in der Hetzner-Produktiv-`.env`** (sonst „Login nicht konfiguriert"); `AUTH_SECRET`=`openssl rand -base64 48`; **echtes Produktiv-Passwort** wählen (Dev-PW `cert-expert-2026` in `.env.local`, gitignored). (3) Optional als nächster Schritt: User-/Rollen-Gate (z. B. „geprüft"-Toggle nur Admin) — bewusst NICHT in diesem Zug, separater Slice + Gate.
> **▶ VORHER — GENERATOR-VEREINHEITLICHUNG gestartet (2026-06-15), lokal committet, NICHT gepusht/deployt.** Marks Befund „Mischmasch: kein sauberer Generator, nur Vordefinierte wählbar, Weg fragwürdig" → **Konzept** `GENERATOR_KONZEPT.md` **abgenommen** (Mark, DM1=ein vereinter Generator · DM2=Lücken-Set nur fehlende Pflicht-Docs · DM3–DM9-Empfehlungspaket; Commit `0b003a1`). Bau-Reihenfolge G1→G2→G3→G4 (G5 Firmen-Generierung später). **G1 `1d94bcf`** = freie Einzelauswahl aus vollem Vorlagen-Katalog (alle Rollen-+Bestellungs-Ordner, einzeln togglebar, Suche) statt nur rollen-/set-gebunden; Schnellstart-Set bleibt Vorauswahl. **G1b `f8d6f7d`** = Cross-Rollen-Mix (Doppelrolle SiMa+FK in einem Export): `generate-employee-docs.ts`-Loop additiv über alle gewählten Rollen, primäre `roleId` zuerst, kollisionsfreie Unterordner; pure Logik `cross-role-export.ts` +8 Tests. **Gates: tsc 0 · Suite 206/206 · `next build` grün · EC-09 beide ZIP-Smokes real (Single 94KB wie bisher + Multi 187KB beide Rollen).** Kein Schema-Change (G1/G1b reine Auswahl-/Loop-Schicht), Engine/UE/CL unberührt. **G4 `b891948`** = Top-Level-Generator: neue Route `/generator` + Navbar-Punkt „Generator", Umschalter Firmen-/Mitarbeiter-Dokumente, erreichbar OHNE erst Akte zu öffnen (MA-Bereich: Firma→Person→Deep-Link in Akte-Generator); Tool-1-Body extrahiert (`ModelCreatorWorkspace`), beide ZIP-Pfade real grün. **Fix `ff96b12`** = Nachweis-Upload auffindbar (M4-UX-Regression: Upload-Einstieg jetzt im Nachweise-/Unterweisungen-/Bestellungen-Panel statt verstecktem Header). **▶ OFFEN:** G2 (Lücken-Einstieg: fehlende Pflicht-Docs vormerken) · G3 (Set-Quellen zu einer, P3c) · G4-Folge: `/model-creator` aus Navbar ablösen? + MA-Generator-Workspace echt einbetten statt Deep-Link (EC-09-riskant, eigener Slice). **Abnahme-Befund „von–bis Schulungen" gefixt `7598067`:** mehrtägige Schulung → Dokument-/Trainerdatum = LETZTER Tag (`plannedBis`), Schulungsdauer als Zeitraum angezeigt, „überfällig" auf letzten Tag bezogen (Mark-Vorgabe). **Browser-Klick-Abnahme (Mark) steht aus** für Generator G1/G1b/G4 + Maske M3–M7 + Schulungs-Fix — Code-Gates+ZIP grün, visuell noch nicht. Detail je Slice = Executor-Einträge unten.
> **▶ VORHER — AKTE-MASKE M3–M7 FERTIG GEBAUT (2026-06-15), lokal committet, NICHT gepusht/deployt.** Auf Marks „alles fertigbauen (M3–M7)" der Reihe nach gebaut (Konzept `AKTE_MASKE_KONZEPT.md` §8; Marks Empfehlungen DM2–DM9 als verbindlich übernommen, `setKategorie` read-only DM8). Pro Slice Gates grün + committet (Terminal-Planer baute via Executor-Subagenten, Gates selbst re-verifiziert): **M5 `f5bee81`** = `gender` (Sektion A, bei weiblich amber Mutterschutz-Hinweis CL-77, EC-10 kein Auto-Status) + `setKategorie`/Ist-UE als read-only-Verweis (keine Doppel-Editier-Stelle, Ist-UE bleibt bei Schulungszielen). **M3 `bf94f40`** = einheitliche Status-Badges ●vorhanden/○Pflicht-Lücke/◇optional-leer/▲prüfen (zentrale `STATUS_GLYPH`-Map in `EmployeeFileStatusBadge`, ▲ nur aus bestehenden Triggern gender/Dienstfahrzeug/NON-DIN). **M4 `6d22eba`** = Stift je Kapitel (`EmployeeFileChapterEdit`, ersetzt globalen Edit-Toggle für Stammdaten/Rolle&SDL) + vertikaler geführter Anlege-Fluss (`EmployeeFileGuidedCreate`, auf S1b-Basis, „Person speichern" kein Auto-Save); Bestellungen-Upload bleibt bewusst am Nachweis-Toggle (Schicht ④); Generator im Anlege-Fluss ausgeblendet. **M6 `29b17b7` ⚠️ SCHEMA-CHANGE** = 3 additive nullable Spalten am `EmployeeFile` (`exitDate`, `erstunterweisungDatum`, `tallyImportedKeys Json?`) + alle Save/Load/Update-Mappings (`employee-file-repository`) + Intake setzt `tallyImportedKeys` + ◆-Tally-Herkunfts-Badge (neutral-blau, Tooltip „aus vGNvY0, ungeprüft", EC-10) + Austrittsdatum/Erstunterweisung als echte `DatePicker` (Erstunterweisung an vorhandene `defaultErstunterweisungDatum`-Engine-Logik gebunden, **keine UE/CL geändert**). **M7 `b17e9aa`** = Legacy-`EmployeeForm` 1637→836 Zeilen (toter master/full-Stammdaten-Block raus, durch neue Maske ersetzt; EC-09-kritischer „documents"-Doc-Auswahl-/Generator-Pfad **bewusst behalten**). **Gates je Slice: tsc 0 · Suite 198/198 · `next build` grün.** EC-09 real verifiziert (M7-ZIP-Smoke `POST` success, `UEsDBA`/`PK`-Magic, 66 KB, echtes S3). EC-10 gewahrt. **▶ OFFEN für Mark:** (1) **Browser-Klick-Abnahme** der neuen Maske (Stift je Kapitel, geführter Anlege-Fluss, Status-/◆-Badges, neue Datumsfelder) — Code-Gates grün, visuelle Abnahme steht aus. (2) **DEPLOY braucht `db push`** (3 additive Spalten) **+ DB-Backup** vor Migration; danach push. (3) Minor-Fäden (kein Blocker): DM7-Audit-Spur (Tally-Original behalten), ◆ auch in der read-only Ansicht-Card (M2-Cockpit), 7 unused-Local-Warnings in `EmployeeForm`. Detail-Befund je Slice = Executor-Einträge „Von Cursor an Claude" unten.
> **▶ VORHER — AKTE-UMBAU S1a+S1b LIVE (2026-06-13): EINE Akte-Ansicht + Inline-Anlegen.** Marks Befund „Akte aus Tool-2-Queue gewachsen, drei Sichten, Name roh löschbar" → **Zieldesign** `MITARBEITERAKTE_ZIELDESIGN.md` (vertikaler Lebenszyklus Anlegen→Tally→Ausrechnung→Prüfung→Audit; DA1=inline, DA3=Audit nach Projektakte, DA4=S1→S2→S3). **S1a:** Bearbeiten/Übersicht-Umschalter weg → EINE Akte (Ansehen-Standard + akte-weiter „Bearbeiten"-Stift); Person&Rolle nur im Bearbeiten-Modus editierbar (Name/IDs nicht mehr versehentlich löschbar); Bestellungen edit-gegated. **S1b:** „Neue Person" öffnet leeren Akte-Entwurf inline; **kein** Auto-Speichern beim Tippen → expliziter „Person speichern"-Knopf (Name Pflicht) + „Verwerfen"; bleibt danach im Bearbeiten-Modus; keine Geister-Akten. **Browser-verifiziert (Mark, lokal)** inkl. Speichern-Fix. Live `20e6bf9` (kein Schema-Change). Gates: tsc 0 · Suite 198/198 · build grün. EC-09: Generator/ZIP unberührt. **Nächster Schritt: Maske-Konzept** — Sub-Agent erstellt `AKTE_MASKE_KONZEPT.md` (neu gedachte Eingabe-Maske mit allen Anforderungen, Varianten + Empfehlung) → Mark-Gate, dann gebaut. Offene Fäden: S2 (Status-Band), S3 (Prüf-Sektion), DA2 (Generator/Doc-Auswahl-Sektion, wartet auf Marks Input), Tally-Aufräumen `Y5Zq80` + neue Form „Projektunterlagen" (`TALLY_FORMULARE_SOLL.md`, Mark bestätigt ODA/SK/Freigabe).
...
## 🔚 Von Cursor an Claude — Executor (Spur E): M3 Status-Badge-System (●/○/◇/▲), 2026-06-15

**Rolle:** Executor. Gebaut wurde NUR der M3-Auftrag (Herkunfts-/Status-Badge-System §3.5/§3.6). Kein Commit (Mark committet). Kein Schema-/Engine-Change. `onSave`/`applyEmployeePatchWithDocSync` unberührt.

**Stufe-1-Befund (Tally-Herkunft persistiert?): NEIN.** Es gibt KEIN feld-genaues Herkunfts-Flag.
- `EmployeeFile` (Prisma, `prisma/schema.prisma:35–102`) hat keine `source`/`imported`/`tallyResponseId`-Spalte je Feld; ebenso der `Employee`-Typ (`modules/03-mitarbeiterakte-tool-2/employee-file/types/employee.ts:53–172`).
- Der Intake (`lib/tally-intake-service.ts:532–560`) schreibt die Tally-Werte (`fullName`/`birthday`/`roleType`/`employmentType`/`qualification`/`guardIDNumber`/`employeeIDNumber`) direkt in die flachen Spalten — ohne Herkunfts-Marker.
- Einziges Signal: die Akte-`id`-Konvention `tally-{responseId}-emp-{n}` (`tally-intake-service.ts:530`) markiert die **ganze Akte** als Tally-Ursprung, aber NICHT feld-genau, und überlebt manuelle Bearbeitung unverändert (sagt also nichts über einzelne Werte).

**Gebaut (rein ableitbar, kein Schema-Change):**
- **`EmployeeFileStatusBadge.tsx`** additiv um ein Glyph-Präfix erweitert: zentrale `STATUS_GLYPH`-Map leitet ●/○/◇/▲ konsistent aus dem vorhandenen `WorkingItemStatus` ab (`vorhanden`/`vorbereitet`/`beantragt`→● · `fehlt`→○ · `offen`/`nicht erforderlich`→◇ · `unvollständig`/`fachlich prüfen`/`abgelaufen`/`nicht lesbar`→▲). Neues optionales Prop `showGlyph` (default `true`). **Eine** Badge-Quelle für alle 5 Verwender (Edit-Table, DossierView, Overview, EvidenceRow) — kein paralleles System; Farben unverändert (nur Glyph-Präfix additiv).
- **`EmployeeFilePersonRolleEditTable.tsx`** vereinheitlicht: neue `deriveRowStatus(id, hasValue)` + `PFLICHT_ROW_IDS` (`fullName`/`birthday`/`startDate`/`roleId` → row-IDs `vorname`/`nachname`/`geburtsdatum`/`vertragsbeginn`/`dokumenten-vorlage`). Zentrale `ROW_VALUE_PRESENT`-Map (Wert gesetzt?) und `ROW_PRUEFEN`-Map (▲-Trigger). `rowShell` leitet den Status jetzt EINHEITLICH ab: expliziter Override > ▲-Prüf-Trigger > ●/○/◇ aus Wert-Präsenz > Fallback `statusForRow`. Die zuvor uneinheitlichen Ad-hoc-Overrides (`geschlecht`/`norm-klasse`/`set-kategorie`/`ist-ue-verweis`) entfernt → genau eine Quelle.
- **▲ „fachlich prüfen"** nutzt NUR die bestehenden Trigger (keine neuen erfunden): `gender==="weiblich"` (CL-77), `drivesServiceVehicle===true` (CL-73), SDL enthält `non-din` (CL-72).
- **EC-10 gewahrt:** ● = nur „Wert erfasst", keine Erledigt-/Freigabe-/Auditaussage; ▲ = Prüfhinweis, kein Auto-Status.

**◆-Tally-Herkunfts-Badge: GEPARKT (nicht gebaut).** Greift die Entscheidungsregel: ◆ feld-genau bräuchte einen Schema-/Modell-Change (s. Stufe-1-Befund) → in diesem Slice NICHT gebaut, Schema NICHT angefasst.
> **FRAGE an Planer/Mark (M6-Migration?):** Soll ein feld-genaues ◆-Herkunfts-Badge kommen? Minimal-invasiver, additiver Vorschlag: eine nullable Spalte `tallyImportedKeys Json?` am `EmployeeFile` (Array der feld-Keys, die der Import gesetzt hat). Gesetzt an EINER Stelle: im `employeeFile.upsert` in `lib/tally-intake-service.ts` (~Z.532) die tatsächlich befüllten Keys mitschreiben; beim manuellen Überschreiben eines Werts den Key entfernen (Audit-Spur, DM7). Rein additiv (P2023-sicher), keine Engine-Wirkung, ◆ = neutral-blau/grau (nicht Brand-Vermillion), Tooltip „aus Formular vGNvY0, ungeprüft" (EC-10). **Bündeln mit M6-Migration?** — eure Entscheidung, vom Executor NICHT eingeplant.

**Gates:** `tsc --noEmit` = 0 · Test-Suite **198 pass / 0 fail** (`tally-company-intake` · `document-collection-model` · `model-document-plan` · `tool1-company-profile` · `employee-file/*`) · `next build` Compiled successfully · **EC-09-Smoke `GET`+`POST /employee-automation` = 200** (Generator-Pipeline unberührt). **Offen:** finale visuelle Browser-Akzeptanz der Glyphs (Akte öffnen → „Bearbeiten") konnte ich nicht selbst klicken (Chrome-MCP in diesem Lauf nicht aufrufbar) → bitte Mark verifiziert den Reiter visuell.

**Geänderte Dateien:**
- `modules/03-mitarbeiterakte-tool-2/employee-file/EmployeeFileStatusBadge.tsx`
- `modules/03-mitarbeiterakte-tool-2/employee-file/EmployeeFilePersonRolleEditTable.tsx`
