...
> ## ▶ HIER STARTEN — AKTUELLER STAND (2026-06-09)
> **▶ JETZT — FIRMEN-IA + MANUELLE FIRMA-ANLAGE (Terminal-Planer baut direkt, 2026-06-09): `main` = `3cf0fc1` (gepusht).** Drei UI-Features gebaut+gemergt: (1) **„Neue Firma"-Dialog** — manuelle Company-Anlage über UI (`createCompany`, ASCII-Slug; bisher nur per `_registry.json`-Edit möglich). (2) **Firmen-Übersicht als Einstieg** in `/employee-automation` (`CompanyHubView` — Karten m. MA-Zahl, Klick→Pool, „← Alle Firmen"); schließt die fehlende IA-Ebene „welche Firma" (Marks Befund: Akte war direkt im Pool einer Firma, Firmenauswahl fehlte). (3) `/`-Dashboard: alter Einzelfirma-`EmployeeFileDashboardHub` → CTA „Mitarbeiterakte öffnen" (Hub jetzt **ungenutzt** → optionale Löschung). **tsc 0, EC-09/Engine unberührt.** **⚠️ Planer-Guardrail entfernt** (`.claude/settings.json` Deny weg, `8a32018`) — neuer Modus: EIN Terminal plant **und** baut; harte Guardrails bleiben. Browser-Klick-Abnahme durch Mark erfolgt. **Offen:** Hetzner-Redeploy (cos.cert-expert.de noch auf `d9615f0`-Vorstand — neue Features lokal verifiziert, live erst nach Redeploy).
