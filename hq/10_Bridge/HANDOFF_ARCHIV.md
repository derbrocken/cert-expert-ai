# HANDOFF — ARCHIV (Projekt-Historie)

> **Dies ist das Archiv.** Der aktive Briefkasten ist `HANDOFF.md`. Hier liegen die vollständigen historischen Einträge (Slice 0/0a/0b, Slice 1 + Nachzug, Tally-Intake/Webhook, Stabilisierung, frühe Overviews, Slice-2-Bau, ältere Reviews/Freigaben, erledigte Aufgaben). **Nur lesen / nichts löschen** — Gedächtnis = Repo. Stand des Schnitts: 2026-06-07 (Planer 2). Alles ab „UE-Anzeige / Findings 1–5" steht aktiv im `HANDOFF.md`.

---

# HANDOFF — Briefkasten Claude ⇄ Cursor (historischer Volltext, Snapshot)
...
**Verifiziert:** `tsc --noEmit` 0 Fehler (vor UE-Increment; nach Increment letzter Lauf vom User unterbrochen — **Builder 2 bitte `tsc` final bestätigen**) · ESLint geänderte Dateien 0 Errors · **Browser TEIL A:** Liste lädt, **joe sichtbar, kein 500/P2023 mehr** ✅ · **Browser TEIL B:** Schulungs-Soll-Block rendert korrekt (EC-10-Header, CL-11, Soll 40 UE, Balken) ✅.

**OFFEN / Builder 2:**
1. **`tsc --noEmit` final bestätigen** (letzter Lauf unterbrochen) + **EC-09-Smoke** (Generator → ZIP) erneut grün.
2. **Ist-UE-Interaktivität manuell im Browser bestätigen:** Automations-Test konnte das Zahl-Input nicht zuverlässig triggern (bekanntes React-controlled-input-Artefakt — programmatisches `input.value` feuert `onChange` nicht). Code-Pfad ist korrekt (Input → `onChange` → `setIst` → `onSavePerson`/`handleSavePerson` → `setEmployees` synchron → `getEmployeeFileSummary` rechnet Rest=Soll−Ist neu). **Echter manueller Tipp-Test** ausstehend: 20 → „unvollständig"/Rest 20/Balken 50 %; 40 → „rechnerisch erreicht"/Rest 0.
3. **Persistenz** Ist-UE nach Reload prüfen (debounced `saveEmployeeQueue` 400ms + Server).
4. **Commit** (alles uncommitted — s. u.) nach Marks OK.
...
**Dev-Server:** läuft auf :3001 (Hintergrund, **außerhalb Sandbox** starten — sonst `uv_interface_addresses`-Fehler). Genau EINEN: `lsof -ti:3001 | xargs kill -9` vor Neustart.

✅ **Stabiler Punkt** nach Verify+Commit. **Übergabe an Builder 2 empfohlen (Context ~80 %).**
