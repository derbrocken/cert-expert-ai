...
## ✅ DEPLOY AUSGEFÜHRT (2026-06-21, Slice A+C Generator-UX) — `27aa523` → `cb8313d`

> **Live + verifiziert** auf https://cos.cert-expert.de (deployter Commit **`cb8313d`**). Inhalt: **Slice A** (Generator entdoppelt — `/model-creator` entfernt, eine „Generator"-Navbar mit Umschalter Firma/Mitarbeiter) + **Slice C** (Schulungs-von–bis je Person im Generator-Datumsblock, Quelle = `trainingPlan`). **Kein Schema-Change → kein `db push`.** Ablauf: Sicherheits-Backup `dev-20260621-165135.db` → `git pull` (→`cb8313d`) → `npm ci` → `npm run build` (grün) → `systemctl restart` (active). **Live-Checks grün:** `/login` 200 · `/` ohne Cookie 307 · `/model-creator` → 307→Login (Route weg, eingeloggt 404) · `/generator` (eingeloggt) 200 · `/employee-automation` (eingeloggt) 200. **Offen (Mark):** Browser-Klick-Abnahme (eine Generator-Leiste; im Generator je Schulung von–bis sichtbar/editierbar + ein ZIP-Export = EC-09). **Slice B (Vormerken)** wartet weiter auf Marks Vorlagen↔Rolle-Mapping. Konzept: `AKTE_GENERATOR_UEBERGANG_KONZEPT.md`.
