...
function load_form()
{
  if (typeof rpath!="undefined") location.href="formdata://load";
  else {form_obj=document.form;load_applet(1);}
}

function load_form_pre()
{
  if (typeof rpath!="undefined") location.href="formdata://loadpre";
  else {form_obj=document.form_pre;load_applet(1);}
}
...
function submitform(befehl) { form_obj=document.form; submitform1(befehl); }
function submitform_pre(befehl) { form_obj=document.form_pre; submitform1(befehl); }

function submitform1(befehl)
{
   if (befehl=="print") {if (window.print) { window.print(); } else alert("Bitte benutzen Sie die Druck-Funktion Ihres Webbrowsers."); return;}
   if (typeof rpath=="undefined") {load_applet(2);return;}

   erg="";
   for(i=0; i<form_obj.elements.length; i++) {
      if (form_obj.elements[i].type=="checkbox" ||
       form_obj.elements[i].type=="radio") {
           erg+="&="+(form_obj.elements[i].checked? "1" : "0");
           }
      else 
      if (form_obj.elements[i].type=="text" ||
       form_obj.elements[i].type=="textarea") {
           erg+="&="+escape(form_obj.elements[i].value);
           }
      }
//erg=erg.substring(0,1000);
//         alert("formdata://"+befehl+"x"+erg);
...
var netscape = (name.indexOf("mozilla") >=-1 && name.indexOf("msie")==-1 && name.indexOf("opera") == -1);
   ...
   if (netscape) {
	  ...
	  else alert("Bitte benutzen Sie die Druck-Funktion Ihres Webbrowsers."); 
	  return;
    }
...
{
    parent.applet.mode=0;
  	if (parent.applet.test_applet()==0) {
        if (param==1||param==5) {if (!confirm("Bitte suchen Sie im folgenden Dialog den Dateinamen aus, aus dem die Formulardaten gelesen werden sollen.\r\n\r\nFalls das Dialogfenster nicht angezeigt wird, aktualisieren Sie bitte Ihre Java-Software: http://www.java.com/de/"))  return; }
        if (param==2||param==6) {if (!confirm("Bitte geben Sie im folgenden Dialog den Dateinamen an, unter dem die Formulardaten gespeichert werden sollen.\r\n\r\nFalls das Dialogfenster nicht angezeigt wird, aktualisieren Sie bitte Ihre Java-Software: http://www.java.com/de/"))  return; }
    ...
    if (param==5) { //5 bedeutet: GA-Daten speichern
        erg="";
        for(i=0;i<1200;i++) erg+=escape(parent.inp11_merk[i])+"&";
        for(i=0;i<700;i++) erg+=escape(parent.cb11_merk[i])+"&";
   	 	l_text=erg;
   	 	parent.applet.speichern();
    	}
    if (param==6) {
...
function laden_ok()
{
    ...
    for(i=0; i<form_obj.elements.length; i++) {
    	   if (daten.length<i) return;
            if (form_obj.elements[i].type=="checkbox" ||
             form_obj.elements[i].type=="radio") {
                 form_obj.elements[i].checked=daten[i]=="1";
                 }
            else 
            if (form_obj.elements[i].type=="text" ||
             form_obj.elements[i].type=="textarea") {
                 form_obj.elements[i].value=unescape(daten[i]);
                 }
            }
}
